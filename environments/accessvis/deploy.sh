...
CURRENT_STABLE=$( get_aliased_module "${MODULE_NAME}"/access-vis "${CONDA_MODULE_PATH}" )
NEXT_STABLE="${ENVIRONMENT}-${STABLE_VERSION}"
CURRENT_UNSTABLE=$( get_aliased_module "${MODULE_NAME}"/access-vis-unstable "${CONDA_MODULE_PATH}" )
